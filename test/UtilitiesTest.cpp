...
{
	std::string content = Util::read_file("/home/IBT/stratmann/MooViE/test/files/input6_nouse.csv");
...
BOOST_AUTO_TEST_CASE(split)
{
	...
	auto check_vectors = [](const std::vector<std::string>& cmp0, const std::vector<std::string>& cmp1) {
	...
	check_vectors(Util::split(regular, ","), std::vector<std::string>({"M", "o", "o", "V", "i", "E"}));
	check_vectors(Util::split(one_element, ","), std::vector<std::string>({"MooViE"}));
