...
BOOST_AUTO_TEST_CASE(domainaxis)
{
	...
	BOOST_CHECK_EQUAL(axis.get_scale().get_major_intersections(), conf.get_num_major_ticks_va());
	BOOST_CHECK_EQUAL(axis.get_scale().get_major_intersections(), conf.get_num_major_ticks_va());
	...
	BOOST_CHECK_EQUAL(axis.get_histogram().get_num_intervals(), conf.get_num_histogram_classes());
	BOOST_CHECK_CLOSE(axis.get_histogram().get_section_frequency(0), 3. / data.size(), eps);
	BOOST_CHECK_CLOSE(axis.get_histogram().get_section_frequency(1), 1. / data.size(), eps);
	BOOST_CHECK_CLOSE(axis.get_histogram().get_section_frequency(4), 1. / data.size(), eps);
	BOOST_CHECK_CLOSE(axis.get_histogram().get_section_frequency(8), 2. / data.size(), eps);
	BOOST_CHECK_CLOSE(axis.get_histogram().get_section_frequency(9), 4. / data.size(), eps);

	// Test Setters

	start = M_PI, end = M_PI + M_PI_2;
	radius = 10.0, height = 20.0, line_width = 1.0, font_size = 40;
	line_color = Color(.123, .8, .1, .071), fill_color = Color(.94, .81, .7316, .5);
...
BOOST_AUTO_TEST_CASE(codomaingrid)
{
	...
	BOOST_CHECK_EQUAL(grid.get_scale().get_major_intersections(), conf.get_num_major_ticks_cg());
	BOOST_CHECK_EQUAL(grid.get_scale().get_minor_intersections(), conf.get_num_minor_ticks_cg());
	...
	BOOST_CHECK_CLOSE(grid.get_scale().get_extremes(1).first, extremes1.first, eps);
	BOOST_CHECK_CLOSE(grid.get_scale().get_extremes(1).second, extremes1.second, eps);
...
BOOST_AUTO_TEST_CASE(relationelement)
{
	...
	Mapper m0(
				...
				std::make_pair(axis[0].get_start().value(), axis[0].get_end().value())
		   ),
		   m1(
	...
	BOOST_CHECK_CLOSE(elem[0].coord.angle().value(),
			m0.map(row[0].value), eps);
	BOOST_CHECK_CLOSE(elem[0].coord.radius(), axis[0].get_radius(), eps);
	BOOST_CHECK_CLOSE(elem[1].coord.angle().value(),
			m1.map(row[1].value), eps);
