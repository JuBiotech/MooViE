...
void
MainWindow::on_refresh_btn_clicked ()
{
    QString input_loc = m_ui->input_file_txt->text (), output_loc =
            m_ui->output_file_txt->text ();
    ...
    if (!input_loc.isEmpty () && !output_loc.isEmpty ())
    {
        ...
        try
        {
            ...
            // Redo all swaps
            const std::vector<IOList::Swap>& input_swaps =
                    m_input_list->get_swaps ();
            for (size_t i = 0; i < input_swaps.size (); ++i)
            {
                m_scene->swap_inputs (
                            static_cast<std::size_t> (input_swaps[i].before_pos),
                            static_cast<std::size_t> (input_swaps[i].after_pos));
            }
            m_input_list->set_not_dirty ();
            ...
            // Check if the output variable values have been edited
            const std::vector<IOList::Swap>& output_swaps =
                    m_output_list->get_swaps ();
            for (size_t i = 0; i < output_swaps.size (); ++i)
            {
                m_scene->swap_outputs (
                            static_cast<std::size_t> (output_swaps[i].before_pos),
                            static_cast<std::size_t> (output_swaps[i].after_pos));
            }
            m_output_list->set_not_dirty ();
